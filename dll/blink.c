...
int main(int argc, char *argv[]) {
	 ...
	 * Enable GPIO pins
	 */
	if (-1 == pinEnable(POUT) || -1 == pinEnable(PIN)) return(1);

	/*
	 * Set GPIO directions
	 */
	if (-1 == pinMode(POUT, OUT) || -1 == pinMode(PIN, IN)) return(2);

	do {
		/*
		 * Write GPIO value
		 */
		if (-1 == digitalWrite(POUT, repeat % 2)) return(3);
	 ...
	 * Disable GPIO pins
	 */
	if (-1 == pinDisable(POUT) || -1 == pinDisable(PIN)) return(4);
